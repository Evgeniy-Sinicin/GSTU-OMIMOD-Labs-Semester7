...
namespace LAB_6_SOLAE_LLT_EVENT_CS
{
    ...
    {
        private static bool AreThreadsBlocked { get; set; } = false;

        private static int FinishedThreadscCount { get; set; } = 0;

        private delegate bool ThreadsWorkState();
        private static event ThreadsWorkState CanWork;

        private static void Main(string[] args)
        {
            CanWork += () =>
            {
                return !AreThreadsBlocked;
            };
        ...
        private static void SolveSolaes()
        {
            ...
            for (int i = 0; i < _threadsCount; i++)
            {
                ...
                {
                    ThreadPool.QueueUserWorkItem(HandleThread, _solaes.GetRange(i * _solaesCountToThread, _solaesCountToThread));
                }
                else
                {
                    var _remains = _solaesCount % _threadsCount;
                    ThreadPool.QueueUserWorkItem(HandleThread, _solaes.GetRange(i * _solaesCountToThread, _remains == 0 ? _solaesCountToThread : _solaesCountToThread + _remains));
                }
            }

            while (FinishedThreadscCount < _threadsCount);
        ...
        private static void HandleThread(object _solaesObj)
        {
            ...
            AreThreadsBlocked = false;

            FinishedThreadscCount++;
        }
    }
}